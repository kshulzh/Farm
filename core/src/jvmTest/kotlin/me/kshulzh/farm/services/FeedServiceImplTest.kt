...
class FeedServiceImplTest : BasicTest() {
    ...
    fun shouldSaveIngredientType() {
        Mockito.`when`(ingredientTypeRepository.save(any())).thenReturn(IngredientType())
    ...
    fun shouldDeleteIngredientType() {
        Mockito.`when`(ingredientTypeRepository.deleteById(ingredientTypeDtos[0].id!!)).thenReturn(IngredientType())
    ...
    fun shouldFillStorageIfIngredientDoesNotExistInStorage() {
        ...
        Mockito.`when`(ingredientSumRepository.findIngredientSumByTypeId(ingredientDtos[0].ingredientId))
            .thenReturn(null)
    ...
    fun shouldFillStorageIfIngredientExistInStorage() {
        ...
        Mockito.`when`(ingredientSumRepository.findIngredientSumByTypeId(ingredientDtos[1].ingredientId))
    ...
    fun shouldProduce() {
        ...
        Mockito.`when`(ingredientTypeRepository.findById(feedRecipeDtos[0].ingredientId!!))
            .thenReturn(IngredientType().apply {
        ...
        Mockito.`when`(feedRecipeRepository.findById(feedRecipeDtos[0].id!!)).thenReturn(FeedRecipe().apply {
    ...
    fun shouldFeed() {
        ...
        Mockito.`when`(sectionRepository.findById(any())).thenReturn(Section())
