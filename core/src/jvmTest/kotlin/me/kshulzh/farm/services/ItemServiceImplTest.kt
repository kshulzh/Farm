...
class ItemServiceImplTest : BasicTest() {
    ...
    fun shouldRemoveItem() {
        Mockito.`when`(itemRepository.findById(itemDtos[0].id!!)).thenReturn(Item())
    ...
    fun shouldNotRemoveItemWhenItenDoesNotExist() {
        Mockito.`when`(itemRepository.deleteById(any())).thenReturn(null)
    ...
    fun shouldNotFindItemWhenItemDoesNotExist() {
        Mockito.`when`(itemRepository.findById(any())).thenReturn(null)
